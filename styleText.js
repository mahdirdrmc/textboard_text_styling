...
var BBCodeHTML = function() {
    ...
    me.addBBCode('#{TEXT}#', '<h3>{TEXT}</h3>');
    me.addBBCode('**{TEXT}**', '<strong>{TEXT}</strong>');
    me.addBBCode('*{TEXT}*', '<i>{TEXT}</i>');
    me.addBBCode('__{TEXT}__', '<span style="text-decoration:underline;">{TEXT}</span>');
    ...
    me.addBBCode('?{TEXT}?', '<blockquote>{TEXT}</blockquote>');
  };
